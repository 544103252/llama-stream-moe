...
void llama_moe_stream::preload_full_cache() {
    int64_t n_streamed = 0;
    for (const auto & layer_ptr : layers) {
        if (layer_ptr) {
            if (layer_ptr->n_slots != layer_ptr->n_expert) {
                LLAMA_LOG_WARN("%s: full-cache test requires one slot per expert; preload disabled\n", __func__);
                full_cache_test = false;
                return;
            }
            ++n_streamed;
        }
    ...
    LLAMA_LOG_INFO("%s: preloading every expert for %" PRId64 " streamed layers\n",
            __func__, n_streamed);
    ...
    for (auto & layer_ptr : layers) {
        ...
        cv_done.wait(lk, [&] {
            ...
            for (uint32_t slot = 0; slot < sl.n_slots; ++slot) {
        ...
        if (load_failed) {
            GGML_ABORT("MoE expert streaming: full-cache preload failed (I/O error)");
        }
        refresh_expert_map_locked(sl);
        for (uint32_t expert = 0; expert < sl.n_expert; ++expert) {
            GGML_ASSERT(sl.expert_map[expert] == (int32_t) expert);
        }
    }

    LLAMA_LOG_INFO("%s: full-cache Stream MoE test ready in %.2f s\n",
            __func__, (ggml_time_us() - start_us) / 1e6);
}
